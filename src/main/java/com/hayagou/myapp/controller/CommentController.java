...
public class CommentController {
    ...
    @PostMapping("/{postId}")
    public DataResponse comment(@PathVariable long postId, @Valid @ModelAttribute CommentRequestDto commentRequestDto) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String email = authentication.getName();

        return responseService.getResponse(commentService.wirteComment(email,postId,commentRequestDto));
    }

    @ApiOperation(value = "댓글 목록 조회", notes = "댓글 목록을 조회한다.")
    @GetMapping("/{postId}")
    public DataResponse<PaginationDto> getComments(@PathVariable long postId,
                                                   @ApiParam(value = "페이지", defaultValue = "1")@RequestParam int page,
                                                   @ApiParam(value = "size", defaultValue = "10")@RequestParam int size) {
        return responseService.getResponse(commentService.getComments(postId, page, size));
    }
//
//
//    @ApiImplicitParams({
