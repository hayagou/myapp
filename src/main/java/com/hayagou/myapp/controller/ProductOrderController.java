...
public class ProductOrderController {
    ...
    public CommonResult order(@Valid @PathVariable Long productId) {
    ...
    @GetMapping()
    public ListResult<ProductOrderResponseDto> order(@ApiParam(value = "페이지", defaultValue = "1") @RequestParam int page) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String email = authentication.getName();
    ...
    @GetMapping("/admin")
    public ListResult<ProductOrderResponseDto> getAmdinOrder(@ApiParam(value = "페이지", defaultValue = "1") @RequestParam int page) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String email = authentication.getName();
